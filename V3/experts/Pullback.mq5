...
double ValorUltimoFundo = 9999999;
                ...
                const int &spread[])
{
    ...
    for (i = rates_total - MathMax(shouldStartIn, barsToProcess - shouldStartIn); i < rates_total && !IsStopped(); i++)
    {
        ...
        if (close[i] >= close[i - 1] && bufferColors[i - 1] == 0)
        {      // Closing UP?

            bufferWW[i] = bufferWW[i - 1] + vol;
            bufferColors[i] = 0;

            cotacoes[controle] = close[i];
            controle++;
        }

        else if (close[i] >= close[i - 1] && bufferColors[i - 1] == 1)
        {                           // Closing DOWN ?

            ValorUltimoFundo = close[i];
        }
        ...
        else if (close[i] <= close[i - 1] && bufferColors[i - 1] == 0)
        {                           // Closing DOWN ?

            ValorUltimoTopo = close[i];
        }
...
void buscarFundoTopo()
{
    ...
    if (controle > 0)
    {
        ...
        for (x = 0; x <= controle; x++)
        {
            ...
            if (cotacoes[x] > ValorUltimoTopo) ValorUltimoTopo = cotacoes[x];
            if (cotacoes[x] < ValorUltimoFundo) ValorUltimoFundo = cotacoes[x];
